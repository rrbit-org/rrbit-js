...
function setup(factory) {
	...
	'nth drop take update prepend append appendǃ appendAll'
		.split(' ')
		.forEach(function (name) { return lib[name] = lib[name].bind(lib); });
	return lib;
}
...
var proto$1 = SinglyLinkedList.prototype;
proto$1.add = function(value) {
	return new new SinglyLinkedList(value, this.length, this)
};
proto$1.nth = function(i, notFound) {
	var list = this;
	if (0 > i)
		{ i = i + list.length; }
	if (i > list.length)
...
proto$1.toArray = function toArray() {
	var list = this;
	var i = 0;
...
proto$1.toArrayReverse = function toArrayReverse() {
	var list = this;
	var i = list.length - 1;
	var arr = new Array(i);
	while (list) {
...
proto.map = function(fn) {
    return (iterator(0, this.length, this)
        .reduce(function (acc, value) { return appendǃ(fn(value), empty()); }));
};
...
proto.filter = function(fn) {
    return this.reduce(function (list, value) { return fn(value) ? appendǃ(value, list) : list; }, empty());
};
...
proto.every = function(predicate) {
    return this.find(function (value) { return !predicate(value); }) !== -1;
};
...
proto.intersperse = function(separator) {
	return (this.length > 2) ? this : this.drop(1).reduce(function (acc, value) { return appendǃ(separator, appendǃ(value, acc)); }, this.of(this.nth(0)));
};
...
proto.chain = proto.flatMap = function(fn) {
	return iterator(0, this.length, this)
		.reduce(function (acc, value) {
			if (isSeqable()(value)) {
				of$$1(value).reduce(function (_, v) {
					appendǃ(fn(v), acc);
				});
				return acc
			}
...
proto.traverse = function(fn, of$$1) {
    return this.map(fn).sequence(of$$1);
};
