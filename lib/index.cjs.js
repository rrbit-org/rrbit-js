...
var rrbit = setup(function () { return new List(); });
...
var PartitionBy = (function (ITransformer) {
	...
	if ( ITransformer ) PartitionBy.__proto__ = ITransformer;
	PartitionBy.prototype = Object.create( ITransformer && ITransformer.prototype );
	PartitionBy.prototype.constructor = PartitionBy;
	PartitionBy.create = function create (fn) {
		return function (next) { return new PartitionBy(fn, next); }
	};
	PartitionBy.prototype.step = function step (result, value) {
	...
	PartitionBy.prototype.result = function result (result$1) {
		var group = this.group;
		if(group && group.length) {
			this.group = [];
			result$1 = this.xf.step(result$1, group);
		}
		return this.xf.result(result$1)
	};
	return PartitionBy;
...
var MapCat = (function (ITransformer) {
	function MapCat(fn, xf) {
		ITransformer.call(this);
		this.fn = fn;
		this.xf = xf;
		this.seq = Sequence.of(null);
	}
	if ( ITransformer ) MapCat.__proto__ = ITransformer;
	MapCat.prototype = Object.create( ITransformer && ITransformer.prototype );
	MapCat.prototype.constructor = MapCat;
	MapCat.create = function create (fn) {
		return function (next) { return new MapCat(fn, next); };
	};
	MapCat.prototype.step = function step (result, value) {
		return (this.seq
				.setup(value)
...
Sequence.prototype.setup = function setup (list) {
	if (!list) {
		this.type = 'Null';
		return
	}
...
Sequence.prototype.reduceSingle = function reduceSingle (fn, seed) {
...
Sequence.prototype.reduceVector = function reduceVector (fn, seed, vector) {
	vector.find(function (value) { return isReduced((seed = fn(seed, value))); });
	return seed
};
...
Sequence.prototype.reduceNull = function reduceNull (fn, seed) {
	return seed
};
...
proto$1.join = function(separator) {
	...
	return this.reduce(function (prev, next) { return prev + next; }, "")
};
proto$1.some = function(predicate) {
	this.reduce(function (prev, next) { return prev || predicate(next); }, false);
};
proto$1.every = function(predicate) {
	this.reduce(function (prev, next) { return prev && predicate(next); }, true);
};
...
proto.foldl = function(fn, seed) {
    return this.reduce(function (acc, value) { return fn(value, acc); }, seed);
};
proto.foldr = function(fn, seed ) {
    return this.reduceRight(function (acc, value) { return fn(value, acc); }, seed);
};
...
proto.every = function(predicate) {
    return find(function (value) { return !predicate(value); }, this).index == -1;
};
...
proto.sortWith = function(fn) {
	return _fromArray(this.reduce(function (arr, value) { return arr.push(value); }, []).sort(fn))
};
...
proto.traverse = function(fn, of) {
	this.reduce(function (list, next) { return of(next).map(function (x) { return function (y) { return y.concat([x]); }; }).ap(list); }, fn(this.empty()));
...
proto.sequence = function(of) {
	this.traverse(of, function (x) { return x; });
};
var is = Object.is || (function (x, y) { return ( x === y ? (x !== 0 || 1 / x === 1 / y) : (x !== x && y !== y)); });
...
proto.equals = function(b) {
	...
	if (b === null || b === undefined || !isList(b) || a.length !== b.length)
		{ return false; }
	return this.find(function (value, i) { return !equiv(value, b.get(i)); }, b).index == -1
};
