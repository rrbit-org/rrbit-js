...
function _from(collection) {
    ...
    if (typeof collection.reduce == 'function') {
		return collection.reduce(function (list, value) { return append(value, list); }, empty())
	}
...
proto.map = function(fn) {
    return (this.reduce(function (acc, value) { return append(fn(value), acc); }, empty()));
};
...
proto.appendAll = proto.concat = function(iterable) {
	var addIn = function (list, value) { return append(value, list); };
	var vec = this.reduce(addIn, empty());
	return of$$1(iterable).reduce(addIn, vec)
};
...
proto.intersperse = function(separator) {
	return (this.length < 2) ?
		...
		this.iterator(1, this.length)
			.reduce(function (acc, value) { return append(separator, append(value, acc)); }, this.take(1));
};
